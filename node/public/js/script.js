...
function Export() {
  ...
  // Assuming `data` is available in the scope or fetched from a client-side source
    ...
    course.contents.forEach(module => {
      if (module.summary) {
        ...
        if (startTimeMatch && endTimeMatch) {
          ...
          // After the forEach loop that processes courses and modules
          saveIcalData(cal.toString()).then(() => {
  ...
  async function saveIcalData(icalData) {
    console.log('Generating iCal data...');
    
    console.log('Saving iCal file to directory...');
    const filePath = './schedule.ics'; // Specify the path where you want to save the file
    try {
      await writeFile(filePath, icalData, 'utf8');
      console.log('iCal file saved successfully.');
    } catch (error) {
      console.error('Failed to save iCal file:', error);
    }
  }
}
